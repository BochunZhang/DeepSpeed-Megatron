...
# DeepSpeed Team
#
# ZeRO-Infinity (CPU offload) benchmark on GB200.
# Usage:
#   bash run_zero_infinity.sh                       # default MBS=1
#   MBS=4 bash run_zero_infinity.sh                 # override micro-batch size
#   ENABLE_NSYS=1 bash run_zero_infinity.sh         # with nsys profiling
set -euo pipefail
...
if [ "${ENABLE_NSYS}" -eq 1 ]; then
    NSYS_OUTPUT=${NSYS_OUTPUT:-"nsys_zero_infinity_mbs${MBS}"}
